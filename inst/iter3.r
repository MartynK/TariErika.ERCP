...
# design matrix now is saved after each run and added to prev. results
...
for (i in 1:nrow(des_mat)) {
  ...
  design <-
    rpact::getDesignGroupSequential(kMax = 2,
                                    informationRates = c(des_mat$ir[i],1),
                                    typeOfDesign = "asHSD",
  ...
  simdata <- sim.survdata(N=des_mat$n[i], T=365,
                          censor = des_mat$dropouts[i], # This is the dropout rate
  ...
  for (j in 1:des_mat$nsim[i]) {
    ...
    if (succ_interim == FALSE) {
      ...
      mod3 <- coxph(s~ther, data_full, ties="breslow") # for comparability
      #summary(mod3)
      pval_final <- summary(mod3)$logtest[3]
...
xsts <- file.exists( here::here("data", "des_mat.rds"))

if ( xsts == TRUE) {
  des_mat_old <- readRDS(here::here("data", "des_mat.rds"))
  des_mat <- rbind(des_mat_old, des_mat)
}

saveRDS(des_mat, here::here("data", "des_mat.rds"))
